...
return lerp(input, RestorePostProcess(input, saturate(input), lutColor), injectedData.colorGradeLUTStrength);
}
...
float3 Apply(float3 inputColor, renodx::tonemap::Config tm_config, renodx::lut::Config lut_config, Texture2D lutTexture, bool perChannel){
  if(lut_config.strength == 0.f || tm_config.type == 1.f){
    ...
    color_lut = renodx::lut::Sample(lutTexture, lut_config, color_sdr);
    ...
    color_lut = renodx::lut::Sample(lutTexture, lut_config, saturate(color_hdr));
    return lerp(color_hdr, RestorePostProcess(color_hdr, saturate(color_hdr), color_lut), previous_lut_config_strength);
    }
  }
}
...
float3 applyUserTonemap(float3 untonemapped, Texture2D lutTexture, SamplerState lutSampler, float3 preCompute){
	...
	float midGray = renodx::color::y::from::BT709(renodx::tonemap::ACESFittedAP1(0.18f));	
      ...
      config.reno_drt_flare = 0.05f * pow(injectedData.colorGradeFlare, 5.32192809489);
      ...
      lut_config.strength = 1.f;
        ...
        float3 lutColor = renodx::lut::Sample(lutTexture, lut_config, saturate(hdrColor));
      outputColor = lerp(hdrColor, RestorePostProcess(hdrColor, saturate(hdrColor), lutColor), previous_lut_config_strength);
			} else {
