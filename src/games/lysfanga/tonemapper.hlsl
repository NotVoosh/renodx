...
float3 applyLUT(float3 color, Texture2D lutTexture1, Texture2D lutTexture2, SamplerState lutSampler, float toggle, float3 preCompute){
		float3 output;
		float3 lutColor;
			if(toggle != 0){
		color.rgb = 0.f;
			...
			lutColor = renodx::lut::Sample(saturate(color), lut_config1, lutTexture1);
			output = RestorePostProcess(color, saturate(color), lutColor);
	return output;
}
