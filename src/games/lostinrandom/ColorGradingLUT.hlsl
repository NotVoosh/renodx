...
// from https://github.com/clshortfuse/renodx/blob/main/src/games/poptlc/LUTShared.hlsl
...
float safeDivision(float quotient, float dividend, int fallbackMode = 0)
{
	if (dividend == 0.f) {
        ...
        return renodx::math::FLT_MAX * sign(quotient);
    }
...
float3 Linear_to_PQ(float3 LinearColor, int clampType = 0)
{
	...
	float3 colorPow = pow(LinearColor, renodx::color::pq::M1);
	float3 numerator = renodx::color::pq::C1 + renodx::color::pq::C2 * colorPow;
	float3 denominator = 1.f + renodx::color::pq::C3 * colorPow;
	float3 pq = pow(numerator / denominator, renodx::color::pq::M2);
	if (clampType == 3)
	{
...
float3 PQ_to_Linear(float3 ST2084Color, int clampType = 0)
{
	...
	float3 colorPow = pow(ST2084Color, 1.f / renodx::color::pq::M2);
	float3 numerator = max(colorPow - renodx::color::pq::C1, 0.f);
	float3 denominator = renodx::color::pq::C2 - (renodx::color::pq::C3 * colorPow);
	float3 linearColor = pow(numerator / denominator, 1.f / renodx::color::pq::M1);
	if (clampType == 3)
	{
...
float getDistanceFromCubeEdge(float3 normal)
{
	...
	if (length(normal) <= renodx::math::FLT_MIN)
  {
  ...
  normal *= 3.0;
	...
	float intersectionLength = renodx::math::FLT_MAX;
...
void FixColorGradingLUTNegativeLuminance(inout float3 col, uint type = 1)
{
  ...
  if (luminance < -renodx::math::FLT_MIN)
  {
    if (type == 1)
    {
...
float3 RestoreHue(float3 targetColor, float3 sourceColor, float amount = 0.5)
{
  ...
  if (renodx::color::y::from::BT709(targetColor) <= renodx::math::FLT_MIN)
  {
...
float3 ColorGradingLUTTransferFunctionIn(float3 col, uint transferFunction, bool mirrored = true)
{
  ...
  {
    return mirrored ? renodx::color::srgb::EncodeSafe(col) : renodx::color::srgb::Encode(col);
  }
  else // LUT_EXTRAPOLATION_TRANSFER_FUNCTION_GAMMA_2_2
  {
    return mirrored ? renodx::color::gamma::EncodeSafe(col) : renodx::color::gamma::Encode(col);
  }
}
// Decode.
float3 ColorGradingLUTTransferFunctionOut(float3 col, uint transferFunction, bool mirrored = true)
{
  if (transferFunction == LUT_EXTRAPOLATION_TRANSFER_FUNCTION_SRGB)
  {
    return mirrored ? renodx::color::srgb::DecodeSafe(col) : renodx::color::srgb::Decode(col);
  }
  else // LUT_EXTRAPOLATION_TRANSFER_FUNCTION_GAMMA_2_2
  {
    return mirrored ? renodx::color::gamma::DecodeSafe(col) : renodx::color::gamma::Decode(col);
  }
}
...
LUTExtrapolationSettings DefaultLUTExtrapolationSettings()
{
  ...
  settings.whiteLevelNits = 100.0f;
...
float3 SampleLUTWithExtrapolation(LUT_TEXTURE_TYPE lut, SamplerState samplerState, LUTExtrapolationData data /*= DefaultLUTExtrapolationData()*/, LUTExtrapolationSettings settings /*= DefaultLUTExtrapolationSettings()*/)
{
  ...
  //TODOFT4: verify this doesn't cause black dots in output due to normalizing smallish vectors (in quality mode 0)
	const bool uvOutOfRange = distanceFromUnclampedToClampedUV > renodx::math::FLT_MIN; // Some threshold is needed to avoid divisions by tiny numbers
  ...
  float3 outputSample = clampedSample;
    ...
    {
      const float maxExtrapolationColor = max((settings.inputTonemapToPeakWhiteNits / settings.whiteLevelNits) - 1.0, renodx::math::FLT_MIN);
...
// This is already explained in the settings description so we disabled the safety check.
    ...
    if (backwardsAmount < lutTexelRange)
    {
      backwardsAmount = lutTexelRange;
		...
		const float PQNormalizationFactor = 10000.0f / settings.whiteLevelNits;
...
#if 1
      const bool secondSampleLessCentered = backwardsAmount > (0.25 + renodx::math::FLT_MIN);
    ...
    // See the setting description for more information
      ...
      float extrapolatedSampleLuminance = renodx::color::y::from::BT709(extrapolatedSample);
      float clampedSampleLuminance = renodx::color::y::from::BT709(clampedSample);
