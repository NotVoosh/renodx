...
float3 applyUserTonemapACES(float3 untonemapped, int curve = 0, bool grading = true, float peak = 0) {
  float3 outputColor;
  float midGray = vanillaACES(float3(0.18f, 0.18f, 0.18f), curve).x;
  float3 hueCorrectionColor = vanillaACES(untonemapped, curve);
  float defaultClip;
  if(curve == 2){
    defaultClip = 26.f;
  } else if (curve == 4){
  ...
  if (grading) {
    ...
    config.reno_drt_blowout = 1.f - injectedData.colorGradeBlowout * 0.85f;
  } else {
    config.reno_drt_blowout = 0.1f;
  }
...
float3 applyUserTonemapNeutral(float3 untonemapped, bool grading = true, float peak = 0) {
  float3 outputColor;
