  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.yw = float2(0,0.25);
  float3 preCG = r2.xyz;
  float y = renodx::color::y::from::BT709(preCG);
  float3 sdrColor = lerp(preCG, renodx::tonemap::renodrt::NeutralSDR(preCG), saturate(y));
  ...
  if (injectedData.toneMapType != 0.f) {
    ...
    r3.xyz = renodx::tonemap::UpgradeToneMap(preCG, min(1.f, userLutInput), r3.xyz, injectedData.colorGradeUserLUTStrength);
  }
  ...
  y = renodx::color::y::from::BT709(hdrColor);
  sdrColor = lerp(hdrColor, renodx::tonemap::renodrt::NeutralSDR(hdrColor), saturate(y));
