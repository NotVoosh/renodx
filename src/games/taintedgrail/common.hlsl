...
float4 PostToneMapScale(float4 color) {
  if (injectedData.toneMapGammaCorrection == 2.f) {
    color = renodx::color::correct::GammaSafe(color, false, 2.4f);
    color *= injectedData.toneMapGameNits / injectedData.toneMapUINits;
    color = renodx::color::correct::GammaSafe(color, true, 2.4f);
...
float3 vanillACES(float3 color) {
...
float3 applyUserTonemapMenuCES(float3 untonemapped) {
  ...
  if (injectedData.toneMapType == 0.f) {
    outputColor = vanillACES(untonemapped);
  } else {
...
float3 applyUserTonemapMenuCustom(float3 untonemapped, float3 vanilla, float midGray) {
  float3 outputColor;
  renodx::tonemap::Config config = renodx::tonemap::config::Create();
  ...
  config.reno_drt_white_clip = 3.f;
  if (injectedData.toneMapType == 0.f) {
...
float3 applyUserTonemapMenuClip(float3 untonemapped) {
  float3 outputColor;
  renodx::tonemap::Config config = renodx::tonemap::config::Create();
  config.type = injectedData.toneMapType > 1.f ? 3.f : injectedData.toneMapType;
  config.peak_nits = injectedData.toneMapPeakNits;
