...
Texture3D<float4> t4 : register(t4);  // 16³ LUT
Texture3D<float4> t3 : register(t3);  // 16³ LUT
Texture2D<float4> t2 : register(t2);
Texture2D<float4> t1 : register(t1);  // likely depth
Texture2D<float4> t0 : register(t0);  // tonemapped
  ...
  out float4 o0 : SV_Target0)
{
  ...
  if (r0.z != 0) {
    ...
    r2.gba = renodx::tonemap::config::Apply(outputColor, config);
        if(config.type == 4.f){
		config.shadows *= 0.8f;
  ...
  if (r0.z != 0) {
    ...
    r2.yzw = cb1[2].yyy * r4.xyz + r3.xyz;
      ...
      r2.gba = lerp(lutColor1, lutColor2, cb1[2].y);
      r2.gba = renodx::color::gamma::DecodeSafe(r2.gba);
  } else {
    r0.z = cmp(0 < cb1[2].x);
    if (r0.z != 0) {
      r2.yzw = saturate(r2.yzw);
    ...
    bool perChannelCombo = config.type == 3.f && config.reno_drt_per_channel == true || config.type == 2.f; 
      if(config.type == 0.f){
    r2.gba = lerp(lutInput, lutOutput, previous_lut_config_strength);
    } else if (perChannelCombo == true && injectedData.upgradePerChannel == 1.f){
    r2.gba = UpgradeToneMapPerChannel(hdrColor, sdrColor, lutOutput, previous_lut_config_strength);
    } else {
  ...
  r3.xyz = saturate(r3.xyz * cb1[0].www + float3(0.730000019,0.730000019,0.730000019));
    if(injectedData.toneMapType != 0.f && cb1[0].x > 0){
    r3.rgb = renodx::color::gamma::Decode(r3.rgb);
    outputColor = UpgradeToneMapPerChannel(linearColor, saturate(linearColor), r3.rgb, 1.f);
    r3.rgb = renodx::color::gamma::EncodeSafe(outputColor);
    }
