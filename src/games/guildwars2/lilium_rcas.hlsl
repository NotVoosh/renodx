...
#define SHARPENING_NORMALIZATION_POINT (injectedData.toneMapPeakNits / injectedData.toneMapGameNits)
  ...
  //    h
  float3 b =
      renodx::color::srgb::DecodeSafe(SamplerFrameBuffer_TEX.SampleLevel(SamplerFrameBuffer_SMP_s, tex_coord + float2(0, -1) * texel_size, 0).rgb);
  float3 d =
      renodx::color::srgb::DecodeSafe(SamplerFrameBuffer_TEX.SampleLevel(SamplerFrameBuffer_SMP_s, tex_coord + float2(-1, 0) * texel_size, 0).rgb);
  float3 e =
      center_color;
  float3 f =
      renodx::color::srgb::DecodeSafe(SamplerFrameBuffer_TEX.SampleLevel(SamplerFrameBuffer_SMP_s, tex_coord + float2(1, 0) * texel_size, 0).rgb);
