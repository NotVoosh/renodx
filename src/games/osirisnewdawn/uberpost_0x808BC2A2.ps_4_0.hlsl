...
// ---- Created with 3Dmigoto v1.3.16 on Thu Sep 26 07:58:30 2024
Texture2D<float4> t8 : register(t8);        // grain/noise

Texture2D<float4> t7 : register(t7);

Texture3D<float4> t6 : register(t6);        // LUT

Texture2D<float4> t5 : register(t5);        // chromatic abberration spectrum

Texture2D<float4> t4 : register(t4);        // lens dirt texture

Texture2D<float4> t3 : register(t3);        // bloom

Texture2D<float4> t2 : register(t2);        // ?? some full red screen ??

Texture2D<float4> t1 : register(t1);        // color + DoF

Texture2D<float4> t0 : register(t0);        // LDR LLL (exposure?)
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r4.w = 0.25 * r0.w;
  r0.xyzw = r4.xyzw * injectedData.fxBloom + r1.xyzw;       // bloom
  r1.xyz = r2.xyz * r3.xyz;
  r1.w = 0;
  r0.xyzw = r1.xyzw * injectedData.fxLensFlare + r0.xyzw;              // lens flare
  ...
  r2.xyz = injectedData.fxFilmGrainType ? r1.yzw : r0.xyz * r0.www * injectedData.fxFilmGrain + r1.yzw;         // vanilla grain
