...
float3 vanillaTonemapSteep(float3 untonemapped, float brightness = -0.08571427f, float contrast = 0.88f) {
  float3 color = max(0, max(0, untonemapped) - 0.04f);
  float3 num = color * (color * 3.2f + 0.5f);
  float3 den = color * (color * 3.2f + 0.5f) + 1.06f;
  float3 gammaColor = pow(max(0.00001f, num/den), 1 / 2.2f);
  gammaColor = gammaColor + brightness;
  gammaColor = saturate(lerp(float3(0.5,0.5,0.5), gammaColor, contrast));
  return pow(gammaColor, 2.2f);
}

float3 applyUserTonemapSteep(float3 untonemapped, Texture3D lutTexture, SamplerState lutSampler) {
  float3 outputColor = untonemapped;
  float midGray = renodx::color::y::from::BT709(vanillaTonemapSteep(float3(0.18f, 0.18f, 0.18f)));
  float3 hueCorrectionColor = (vanillaTonemapSteep(outputColor));
  ...
  if (injectedData.toneMapType == 4.f) {  // Reinhard+
    ...
    float3 lutColor = Sample(lutTexture, lut_config, sdrColor);
