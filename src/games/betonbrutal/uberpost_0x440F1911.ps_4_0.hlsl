...
Texture2D<float4> t6 : register(t6);  // 16x16 something

Texture3D<float4> t5 : register(t5);  // log lut 33³

Texture2D<float4> t4 : register(t4);  // CA

Texture2D<float4> t3 : register(t3);  // dirt

Texture2D<float4> t2 : register(t2);  // bloom

Texture2D<float4> t1 : register(t1);  // white
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r2.xyz = injectedData.fxFilmGrainType ? r1.gba : r0.xyz * r0.www + r1.yzw;
  ...
  r1.xyzw = t5.Sample(s5_s, r0.xyz).wxyz;
    r1.gba = applyUserTonemap(untonemapped, t5, s5_s);
